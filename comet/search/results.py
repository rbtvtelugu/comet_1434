...
class SearchResult:
    ...
    def matches_title(self, title: str, year: int):
        if self.parsed.parsed_title and not title_match(title, self.parsed.parsed_title):
            return False
        if year and self.parsed.year and year != self.parsed.year:
            return False
        return True
